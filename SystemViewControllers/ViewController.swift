...
//
//  Created by Артем Мартиросян on 08/03/2019.
...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, MFMailComposeViewControllerDelegate, MFMessageComposeViewControllerDelegate {
    ...
    @IBAction func safariButtonTapped(_ sender: UIButton) {
        if let url = URL(string: "http://www.instagram.com/syanches") {
    ...
    @IBAction func messageButtonTapped(_ sender: UIButton) {
        guard MFMessageComposeViewController.canSendText() else {
            print("You can't send text messages")
            return
        }
        ...
        messageComposer.recipients = ["+79998081001"]
        messageComposer.body = "Hello! I try to send message via my app!"
